...
net = detectNet("ssd-mobilenet-v2", threshold=0.5,
                ...
                model="asl_detect/ssd-mobilenet.onnx",
                labels="asl_detect/labels.txt",
                input_blob="input_0", output_cvg="scores", output_bbox="boxes")
camera = videoSource("/dev/video0")      # '/dev/video0' for V4L2
display = videoOutput("webrtc://@:8554/my_output") # 'my_video.mp4' for file

# while display.IsStreaming():
# NOTE: apparently a WebRTC connection reports isStreaming as `False` until a client connects,
# so instead let's just loop forever (until Ctrl+C)
while True: 
   img = camera.Capture()
